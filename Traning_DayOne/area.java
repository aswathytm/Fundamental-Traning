...
public class area
{
        ...
        Shape objectFour = new Shape();
        Print objectFive=new Print(); // abstraction
        while(true){
        objectFive.list();
        objectFour.setShape(sc.nextInt());
        int value = objectFour.getShape();
        ...
        objectOne.Area(length,Width); // Polymorphism
        }
        if(value==2) // Finding Area of Square
        {
        System.out.println("Enter the Length of squre :");
        int length= sc.nextInt();
        Square objectTwo = new Square();
        objectTwo.Area(length);
        }
        ...
        Triangle objectThree = new Triangle();
        objectThree.Area(sideOne,sideTwo,sideThree);
        }
